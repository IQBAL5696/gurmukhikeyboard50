...
object ImeConstants {
    const val ACTION_SETTINGS_CHANGED = "com.iqbal.gurmukhikeyboard50.SETTINGS_CHANGED"
    const val PREF_KEYBOARD_THEME = "pref_keyboard_theme"
    const val PREF_CURRENT_PANEL = "pref_current_panel"
    const val PREF_DEFAULT_KEYBOARD = "pref_default_keyboard"
    const val PREF_EXPANDED_TOP_ROW_BUTTONS = "pref_expanded_top_row_buttons"
    const val PREF_KEY_HEIGHT = "pref_key_height"
    const val PREF_FONT_SIZE = "pref_font_size"
    const val PREF_KEY_GAP = "pref_key_gap"
    ...
    const val KEYCODE_TRANSLATE = -123
    const val KEYCODE_LANGUAGE_SWITCH = -124
    const val KEYCODE_VOICE_INPUT = -125
    const val KEYCODE_GURBANI_PLAYER = -126
    const val KEYCODE_EMOJI = -127
    ...
    const val PANEL_TRANSLATION = 3
    const val PANEL_EMOJI = 4
