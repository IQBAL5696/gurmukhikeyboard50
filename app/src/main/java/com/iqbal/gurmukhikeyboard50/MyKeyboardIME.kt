...
class MyKeyboardIME : InputMethodService(), SharedPreferences.OnSharedPreferenceChangeListener, VoiceRecognitionResultListener, TextToSpeech.OnInitListener {
    ...
    override fun onDestroy() { super.onDestroy(); sharedPreferences.unregisterOnSharedPreferenceChangeListener(this); voiceInputManager.stopVoiceRecognition(); translationManager.close(); tts?.stop(); tts?.shutdown(); serviceScope.cancel() }
    ...
    override fun onCreateInputView(): View {
        ...
        keyboardActionListener = MyKeyboardActionListener(this); kv?.setOnKeyboardActionListener(keyboardActionListener); customizeTopRowButtons(themedContext); nanakshahiCalendarPanel = NanakshahiCalendarPanel(themedContext, { switchPanel(ImeConstants.PANEL_KEYBOARD) }); (nanakshahiCalendarPanelContainer as? FrameLayout)?.addView(nanakshahiCalendarPanel?.view); setupTranslationPanel(themedContext); setupAiAssistantPanel(themedContext); setupEmojiPanel(themedContext); setupClipboardPanel(themedContext); setupExpandedTopRow(themedContext)
        root.findViewById<ImageButton>(R.id.btn_move_to_left).setOnClickListener { sharedPreferences.edit().putString("pref_one_handed_mode", "left").apply() }; root.findViewById<ImageButton>(R.id.btn_move_to_right).setOnClickListener { sharedPreferences.edit().putString("pref_one_handed_mode", "right").apply() }; root.findViewById<ImageButton>(R.id.btn_expand_left).setOnClickListener { sharedPreferences.edit().putString("pref_one_handed_mode", "off").apply() }; root.findViewById<ImageButton>(R.id.btn_expand_right).setOnClickListener { sharedPreferences.edit().putString("pref_one_handed_mode", "off").apply() }
        applyOneHandedMode(); switchPanel(ImeConstants.PANEL_KEYBOARD, forceRedraw = true); return root
    }
    ...
    private fun handleSuggestionClick(suggestion: String) {
        ...
        if (suggestion.isNotEmpty() && !suggestion.startsWith("=") && suggestion != "💬 WhatsApp") serviceScope.launch(Dispatchers.IO) { databaseHelper.addWord(suggestion.trim()) }
    }
    ...
    private fun customizeTopRowButtons(context: Context) {
        ...
        val buttonOrder = listOf(ImeConstants.KEYCODE_EMOJI, ImeConstants.KEYCODE_GURBANI_PLAYER, ImeConstants.KEYCODE_VOICE_INPUT, ImeConstants.KEYCODE_LANGUAGE_SWITCH, ImeConstants.KEYCODE_TRANSLATE, ImeConstants.KEYCODE_AI_ASSISTANT, ImeConstants.KEYCODE_OCR, ImeConstants.KEYCODE_SWITCH_TO_NANAKSHAHI_CALENDAR_PANEL, -105, ImeConstants.KEYCODE_SETTINGS)
    ...
    override fun onSharedPreferenceChanged(sp: SharedPreferences?, key: String?) { if (key == ImeConstants.PREF_KEYBOARD_THEME || key == ImeConstants.PREF_KEY_HEIGHT || key == ImeConstants.PREF_KEY_GAP || key == "pref_one_handed_mode") setInputView(onCreateInputView()) else if (key == "font_size_abs" || key == "popup_on_keypress") updateKeyboardSettings() }
    private fun updateKeyboardSettings() { val popupOn = sharedPreferences.getBoolean("popup_on_keypress", true); kv?.isPreviewEnabled = popupOn; kv?.setKeyTextSize(sharedPreferences.getInt("font_size_abs", 50).toFloat()) }
    override fun onStartInputView(editorInfo: EditorInfo?, restarting: Boolean) { super.onStartInputView(editorInfo, restarting); updateKeyboardSettings(); updateEnterKey(editorInfo); val inputType = editorInfo?.inputType ?: 0; if ((inputType and InputType.TYPE_MASK_CLASS) == InputType.TYPE_CLASS_NUMBER) handleKeyboardSwitch(KeyboardType.NUMPAD) else if ((inputType and InputType.TYPE_MASK_VARIATION) == InputType.TYPE_TEXT_VARIATION_PASSWORD) handleKeyboardSwitch(KeyboardType.ENGLISH) else switchPanel(ImeConstants.PANEL_KEYBOARD, forceRedraw = true); kv?.post { kv?.requestLayout(); kv?.invalidate() }; gurmukhiInputHandler.reset(); if (::keyboardActionListener.isInitialized) keyboardActionListener.resetBuffers(); refreshClipboardHistory(); applyOneHandedMode() }
    ...
    internal fun switchPanel(panel: Int, forceRedraw: Boolean = false) { 
        ...
        aiAssistantPanelContainer?.visibility = if (panel == ImeConstants.PANEL_AI_ASSISTANT) View.VISIBLE else View.GONE
